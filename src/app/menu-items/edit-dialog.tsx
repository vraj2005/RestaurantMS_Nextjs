...
export default function EditItemDialog({ row, categories, close }: { row: any; categories: any[]; close: () => void }) {
  const qc = useQueryClient();
  const { addToast } = useToast();

  const [name, setName] = useState(row.MenuItemName || "");
  const [price, setPrice] = useState(String(row.MenuItemPrice || ""));
  const [categoryId, setCategoryId] = useState(String(row.MenuCategoryID || ""));
  const [image, setImage] = useState(row.MenuItemImagePath || "");

  const mut = useMutation({
    mutationFn: () => updateMenuItem(row.MenuItemID, {
      MenuItemName: name,
    ...
    onError: (error: any) => {
      ...
      addToast(error.response?.data?.message || "Failed to update menu item", "error");
    }
  });
